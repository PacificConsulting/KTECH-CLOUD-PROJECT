    ...
    dataset
    {
        dataitem("Detailed GST Ledger Entry"; "Detailed GST Ledger Entry")
        {
            ...
            begin
                ...
                //Clear(txtData[36]);  //pcpl-064 25aug2023
                if documentno_1 <> "Document No." then begin
                    tds_entries.Reset;
                    tds_entries.SetRange("Document No.", "Detailed GST Ledger Entry"."Document No.");
                    tds_entries.SetRange("Posting Date", "Posting Date");
                    tds_entries.SetRange("Document Type", tds_entries."Document Type"::Invoice);
                    IF tds_entries.FindSet() then
                        repeat
                            txtData[36] := format(tds_entries."TCS Amount");
                        until tds_entries.Next() = 0;
                end;
                ...
                /* IF "Detailed GST Ledger Entry"."GST Component Code" = 'IGST' THEN BEGIN
                    AmtToCustomer := Round(TDSDecimal + (ABS("GST Base Amount")) + (ABS("Detailed GST Ledger Entry"."GST Amount")), 1, '>') // PCPL-065 19-JULY-23
                end else
                    AmtToCustomer := Round(TDSDecimal + (ABS("GST Base Amount")) + (ABS("Detailed GST Ledger Entry"."GST Amount" * 2)), 1, '>');// PCPL-065 19-JULY-23
 */
                IF "Detailed GST Ledger Entry"."GST Component Code" = 'IGST' THEN BEGIN
                    AmtToCustomer := Round((TDSDecimal) + (ABS("GST Base Amount")) + (ABS("Detailed GST Ledger Entry"."GST Amount")), 1) // PCPL-064 31-JULY-23
                end else
                    AmtToCustomer := Round((TDSDecimal) + (ABS("GST Base Amount")) + (ABS("Detailed GST Ledger Entry"."GST Amount" * 2)), 1);// PCPL-064 31-JULY-23
    ...
    var
        documentno_1: Code[20]; //25aug2023
